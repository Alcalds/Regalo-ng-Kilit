...
/* =========================================================
   V1.13 — GET INVOLVED PATHWAY SELECTION
   The route cards behave as a lightweight participation hub. Choosing a path
   preselects the matching inquiry category and moves the visitor to the form.
   Wix Studio equivalent: route buttons -> anchor/form section + Wix Form field.
   ========================================================= */
...
if (involvementRoutes.length && inquiryForm && inquiryInterest) {
  involvementRoutes.forEach((button) => {
    button.addEventListener("click", () => {
      const interest = button.dataset.interestTarget || "";
      const optionExists = Array.from(inquiryInterest.options).some((option) => option.value === interest || option.textContent.trim() === interest);
      ...
      document.querySelectorAll(".involvement-path").forEach((card) => card.classList.remove("is-selected"));
      const selectedCard = button.closest(".involvement-path");
